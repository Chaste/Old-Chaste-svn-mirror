...
public:
    ...
    void TestGetNewNodeLocationForCylindricalMesh() throw (Exception)
    {        
        ...
        CryptSimulation2DPeriodic simulator(*p_mesh, cells);
        simulator.SetOutputDirectory("Crypt2DMoveNodeTest");
        ...
        std::vector< c_vector<double, 2> > dr_dt;
        c_vector<double, 2> this_node_speed;
        // bottom row (move first cell over boundary and onto right)
        // starts at 0,-1.7...
        this_node_speed[0] = -0.01;
        this_node_speed[1] = 0.0;
        dr_dt.push_back(this_node_speed);
        // starts at 1,-1.7...
        this_node_speed[0] = 0.01;
        this_node_speed[1] = 0.0;
        dr_dt.push_back(this_node_speed);
        // starts at 2,-1.7...
        this_node_speed[0] = 0.0;
        this_node_speed[1] = 0.0;
        dr_dt.push_back(this_node_speed);
        // next row (move last cell over boundary and onto left)
        // starts at 0.5,-0.866...
        this_node_speed[0] = 0.0;
        this_node_speed[1] = 0.0;
        dr_dt.push_back(this_node_speed);
        ...
        this_node_speed[1] = 0.0;
        dr_dt.push_back(this_node_speed);
        // starts at 2.999999,-0.866...
        this_node_speed[0] = 0.01;
        this_node_speed[1] = 0.0;
        dr_dt.push_back(this_node_speed);
        new_point = simulator.GetNewNodeLocation(0u, dr_dt);
        //std::cout << " x = " << new_point.rGetLocation()[0] << ", y = " << new_point.rGetLocation()[1] << "\n" << std::flush;
        // This node was on left and is now near the right
        TS_ASSERT_DELTA(new_point.rGetLocation()[0], 3.0, 1e-2);
        
        new_point = simulator.GetNewNodeLocation(1u, dr_dt);
        //std::cout << " x = " << new_point.rGetLocation()[0] << ", y = " << new_point.rGetLocation()[1] << "\n" << std::flush;
        // This node has stayed close to where it was
        TS_ASSERT_DELTA(new_point.rGetLocation()[0], 1.0, 1e-2);
        
        new_point = simulator.GetNewNodeLocation(5u, dr_dt);
        //std::cout << " x = " << new_point.rGetLocation()[0] << ", y = " << new_point.rGetLocation()[1] << "\n" << std::flush;
        // This node was on right and is now on the left.
        TS_ASSERT_DELTA(new_point.rGetLocation()[0], 0.0, 1e-2);
