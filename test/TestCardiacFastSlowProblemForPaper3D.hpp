...
#ifndef TESTCARDIACDOMAINFASTSLOWPROBLEMFORPAPER3D_HPP_
#define TESTCARDIACDOMAINFASTSLOWPROBLEMFORPAPER3D_HPP_
...
template <class CELL>
class MyCellFactory : public AbstractCardiacCellFactory<3>
{
private:
...
class TestCardiacFastSlowProblemForPaper3D : public CxxTest::TestSuite
{
private:
    ...
    MixedTetrahedralMesh<3,3> mMixedMesh;
    TetrahedralMesh<3,3> *mpFineMesh;
    ...
    void RunAndCheckMonodomain(AbstractCardiacCellFactory<3>* pCellFactory,
                               std::string outputDirectory)
    {

        MonodomainProblem<3> monodomain_prob(pCellFactory);
    ...
    void RunAndCheckBidomain(AbstractCardiacCellFactory<3>* pCellFactory,
                                std::string outputDirectory)
    {
        std::vector<double> voltage_normal;
        HeartConfig::Instance()->SetOutputDirectory(outputDirectory);

        BidomainProblem<3> bidomain_prob( pCellFactory);
...
public:
    void TestConstructMixedMesh()
    {
        ...
        TS_ASSERT_EQUALS(num_coarse_elem_each_dir, 3U);
        TS_ASSERT_EQUALS(num_fine_elem_each_dir, 30U);
        mMixedMesh.ConstructCuboidMeshes(width, height, depth, num_coarse_elem_each_dir, num_fine_elem_each_dir);
        mpFineMesh=mMixedMesh.GetFineMesh();

        TS_ASSERT_EQUALS(mpFineMesh->GetNumNodes(), 29791U);
        TS_ASSERT_EQUALS(mMixedMesh.GetNumNodes(), 64U);

        unsigned node_ratio = mpFineMesh->GetNumNodes() / mMixedMesh.GetNumNodes(); // Note -- rounded
        TS_ASSERT_EQUALS(node_ratio, 465U);//Rounded down
        ...
        for (unsigned i=0; i<mpFineMesh->GetNumNodes(); i++){
            double z=mpFineMesh->GetNode(i)->rGetLocation()[2];
            if ( fabs(z - (depth/2.0)) < 1e-6)
            {
                double y=mpFineMesh->GetNode(i)->rGetLocation()[1];
                if ( fabs(y - (height/2.0)) < 1e-6)
                {
                    //Half way up (middle of y,z plane)
        ...
        TS_ASSERT_EQUALS(mMiddleIndex, 14895U);
        TS_ASSERT_EQUALS(mRhsIndex, 14910U);

        mProbeDistance = mpFineMesh->GetDistanceBetweenNodes(mMiddleIndex, mRhsIndex);
        TS_ASSERT_DELTA(mProbeDistance, 0.15, 1e-10);
    ...
    void TestBidomainFastSlowProblemAgainstNormal() throw (Exception)
    {
        ...
        //////////////////////////////////////////////////
        HeartConfig::Instance()->SetOutputDirectory("BidomainFastSlow3D");
        HeartConfig::Instance()->SetOutputFilenamePrefix("res");

        {
            MyCellFactory<FastSlowBackwardEulerNoble98> cell_factory;

            BidomainFastSlowProblem<3> bidomain_fast_slow_prob( &cell_factory, mMixedMesh, mSlowOdeTimestep );
        ...
        RunAndCheckBidomain(&cell_factory_normal, "BidomainBackwardToCompareWithFastSlow3D");
        ...
        if (mRunForwardTests)
        {
            ...
            RunAndCheckBidomain(&cell_factory_lu, "BidomainLookupToCompareWithFastSlow3D");

            MyCellFactory<CML_noble_varghese_kohl_noble_1998_basic> cell_factory_fe;
            RunAndCheckBidomain(&cell_factory_fe, "BidomainForwardEulerToCompareWithFastSlow3D");
        }
        else
        {
            if (PetscTools::AmMaster())
    ...
    void TestMonodomainFastSlowProblemAgainstNormal() throw (Exception)
    {
        ...
        // solve a mixed mesh, fast/slow problem
        //////////////////////////////////////////////////
        HeartConfig::Instance()->SetOutputDirectory("MonodomainFastSlow3D");
        HeartConfig::Instance()->SetOutputFilenamePrefix("res");

         {
            MyCellFactory<FastSlowBackwardEulerNoble98> cell_factory;

            MonodomainFastSlowProblem<3> monodomain_fast_slow_prob( &cell_factory, mMixedMesh, mSlowOdeTimestep );
        ...
        RunAndCheckMonodomain(&cell_factory_backward, "MonodomainBackwardToCompareWithFastSlow3D");
        ...
        if (mRunForwardTests)
        {
            ...
            MyCellFactory<CML_noble_varghese_kohl_noble_1998_basic_pe_lut> cell_factory_lu;
            RunAndCheckMonodomain(&cell_factory_lu, "MonodomainLookupToCompareWithFastSlow3D");

            MyCellFactory<CML_noble_varghese_kohl_noble_1998_basic> cell_factory_fe;
            RunAndCheckMonodomain(&cell_factory_fe, "MonodomainForwardEulerToCompareWithFastSlow3D");
        }
        else
        {
            if (PetscTools::AmMaster())
...
#endif /*TESTCARDIACDOMAINFASTSLOWPROBLEMFORPAPER3D_HPP_*/
