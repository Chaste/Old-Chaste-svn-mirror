function [ this_bucket_height success_prob errors] = probability_of_domination_adhesion_only_playing( input_args )
...
num_heights = length(settings.viscosity_names);

% This is option for how many sub-divisions to use
num_buckets = 1; % Per adhesion height (anything else doesn't make sense)
...
for viscosity_idx = 1:length(settings.viscosity_names)
    ...
    if ~isempty(dataset)
        for j=1:num_buckets
            ...
            % Find only cases of a definite result (only happens at
            % viscosity = 0.6 for some reason!!
            idx = find(dataset(:,4)>-0.5);
            if length(idx)>=num_to_consider
                this_bucket_dataset = dataset(idx(1:num_to_consider),:);
            else
                this_bucket_dataset = dataset(idx,:);
            end

            num_bucket(i) = num_bucket(i) + length(this_bucket_dataset);
            ...
            if (~isempty(idx))
                disp(['No result found for viscosity = ' settings.viscosity_names{viscosity_idx} ])
...
xlabel('Mutant adhesion \alpha','FontSize',16)
...
hold on
errorbar(this_bucket_height,100*success_prob,100*errors,'kx')
xlabel('Mutant adhesion \alpha','FontSize',16)
