...
class BasisFunctionsCheckers
{
							   ...
							   std::vector<Point<SPACE_DIM>*> evaluationPoints)
	{
        ...
        std::vector<double> basis_function_vector_other(SPACE_DIM+1); //store results using WithUpdate method
		...
		for (int point_index=0; point_index<size; point_index++)
		{
            ...
            pBasisFunc->ComputeBasisFunctionsWithUpdate(*(evaluationPoints[point_index]), basis_function_vector_other);
			for (int func_index=0; func_index<size; func_index ++)
			{
                ...
                TS_ASSERT_DELTA(basis_function_vector_other[func_index],
                                expected_evaluation,
                                1e-12);
