...
public:
   ...
   void TestBuildDistributedTetrahedralMeshFromVtkMeshReader(void) throw(Exception)
   {
    ...
    /**
     * Check that reading a VTKUnstructuredGrid into Chaste, then writing it back leaves the file unchanged
     */
    void TestVtkReadThenWriteDoesNotCorruptVtkFile(void) throw(Exception)
    {
#ifdef CHASTE_VTK
        ...
        // Write the mesh back out to file (in VTK format)
        VtkMeshWriter<3,3> mesh_writer("TestVtk", input_mesh, false);
        mesh_writer.WriteFilesUsingMeshReader(mesh_reader);

        TS_ASSERT_EQUALS(system(("diff -a -I \"Created by Chaste\" " + OutputFileHandler::GetChasteTestOutputDirectory() + "TestVtk/" +input_mesh + ".vtu  mesh/test/data/" + input_mesh + ".vtu").c_str()), 0);
#endif // CHASTE_VTK
    }

    /**
     * Check that writing a mesh out in VTK form then, reading it back in results in no changes
     */
    void xTestVtkWriteThenReadGivesSameMesh(void) throw(Exception)
    {
#ifdef CHASTE_VTK
        std::string input_mesh = "cube_2mm_12_elements";

        // Read in a mesh that we know is valid
        TrianglesMeshReader<3,3> triangles_reader("mesh/test/data/" + input_mesh);
        TetrahedralMesh<3,3> mesh_from_triangles;
        mesh_from_triangles.ConstructFromMeshReader(triangles_reader);

        // Write the mesh back in triangles form (to give a Chaste-generated triangles file for comparison)
        TrianglesMeshWriter<3,3> triangles_writer_start("TestVtk/input", input_mesh);
        triangles_writer_start.WriteFilesUsingMesh(mesh_from_triangles);

        // Write the mesh out in VTK format
        VtkMeshWriter<3,3> vtk_writer("TestVtk/vtk", input_mesh, false);
        vtk_writer.WriteFilesUsingMesh(mesh_from_triangles);
        ...
        // Write the mesh back out in triangles format
        TrianglesMeshWriter<3,3> triangles_writer_end("TestVtk/output", input_mesh);
        triangles_writer_end.WriteFilesUsingMesh(mesh_from_vtk);

        // Check that it's the same mesh
        TS_ASSERT_EQUALS( mesh_from_vtk.GetNumNodes(), mesh_from_vtk.GetNumNodes() );
        TS_ASSERT_EQUALS( mesh_from_vtk.GetNumElements(), mesh_from_vtk.GetNumElements() );
        TS_ASSERT_EQUALS( mesh_from_vtk.GetNumBoundaryElements(), mesh_from_vtk.GetNumBoundaryElements() );


        TS_ASSERT_EQUALS(system(("diff -a -I \"Created by Chaste\" " + results_dir + "input/"  + input_mesh + ".node "
                                        + results_dir + "output/" + input_mesh + ".node").c_str()), 0);

        TS_ASSERT_EQUALS(system(("diff -a -I \"Created by Chaste\" " + results_dir + "input/"  + input_mesh + ".ele "
                                        + results_dir + "output/" + input_mesh + ".ele").c_str() ), 0);

        // Face files differ since VTK doesn't store faces explicitly, so we have to re-extract them each time
#endif // CHASTE_VTK
    }
