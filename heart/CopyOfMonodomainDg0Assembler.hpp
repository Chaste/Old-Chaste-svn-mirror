...
class MonodomainDg0Assembler : public AbstractLinearParabolicAssembler<ELEMENT_DIM, SPACE_DIM>
{
...
private:
                           ...
                           Vec currentSolution)
    {
        ...
        for(int quad_index=0; quad_index < 2/*quad_rule.GetNumQuadPoints()*/; quad_index++)
        {
            Point<ELEMENT_DIM> quad_point=rElement.GetNode(quad_index)->GetPoint() /* quad_rule.GetQuadPoint(quad_index)*/;
...
//            exit(0);   
            double wJ = jacobian_determinant * 0.5 ; /*quad_rule.GetWeight(quad_index);  */   
