/**
 * SodiumCurrent.cpp
 * 
 * INa.
 */
#include "SodiumCurrent.hpp"
#include <cmath>

/**
 * Constructor
 * 
 * @param m Initial value for m activation gate
 * @param h Initial value for h inactivation gate
 * @param j Initial value for j inactivation gate
 */
SodiumCurrent::SodiumCurrent()
{   
    mM = 0.0;
    mH = 0.0;
    mJ = 0.0;
    
    mAlphaM = 0.0;
    mAlphaH = 0.0;
    mAlphaJ = 0.0;
    mBetaM = 0.0;
    mBetaH = 0.0;
    mBetaJ = 0.0;
    mMagnitudeOfCurrent = 0.0; // Set to 0.0 initially, its a variable of IonicCurrent class
}

/**
 * Destructor
 */
SodiumCurrent::~SodiumCurrent()
{   
    // Do nothing
}

/**
 * Return value of m activation gate
 */
double SodiumCurrent::GetM()
{   
    return mM;
}

/**
 * Return value of h inactivation gate
 */
double SodiumCurrent::GetH()
{   
    return mH;
}

/**
 * Return value of j inactivation gate
 */
double SodiumCurrent::GetJ()
{   
    return mJ;
}

/**
 * Update gating variables 
 * 
 * @param voltage Current transmembrane voltage
 */
void SodiumCurrent::UpdateAlphaAndBeta(double voltage)
{
    //updating alpha and beta voltage--dependent rate constants
    double mAlphaM = 0.32 * (voltage + 47.13)/(1 - exp(-0.1*(voltage + 47.13)));
    double mBetaM = 0.08 * exp(-voltage/11.0);
    
    if(voltage >= -40.0)
    {
        double mAlphaH = 0.0;
        double mBetaH = 1/(0.13*(exp((voltage+10.66)/(-11.1))+1));
        double mAlphaJ = 0.0;
        double mBetaJ = 0.3*exp(-2.535*0.0000001*voltage)/(1 + exp(-0.1*(voltage+32.0)));
    }
    else
    {
        double mAlphaH = 0.135*exp((80+voltage)/(-6.8));
        double mBetaH = 3.56*exp(0.079*voltage) + 3.1*0.00001*exp(0.35*voltage);
        double mAlphaJ = (-1.2714*0.00001*exp(0.2444*voltage) - 3.474*0.00001*exp(-0.04391*voltage))*(voltage+37.78)/(1+exp(0.311*(voltage+79.23)));
        double mBetaJ = 0.1212*exp(-0.0105*voltage)/(1+exp(-0.1378*(voltage+40.14)));
    }
}

void SodiumCurrent::UpdateGatingVariables(double m, double h, double j)
{  
    mM = m;
    mH = h;
    mJ = j;
}

/**
 * Update magnitude of current, INa.
 * 
 * @param voltage Current transmembrane voltage
 */
void SodiumCurrent::UpdateMagnitudeOfCurrent(double voltage, double m, double h, double j)
{   
    UpdateGatingVariables(m, h, j);
    UpdateAlphaAndBeta(voltage);
    
    // checking how beta is updated
    std::cout << "\n"<< "mBetaM is equal to " <<mBetaM<<std::endl;
    
    mMagnitudeOfCurrent = gNa * pow(mM, 3) * mH * mJ * (voltage - eNa);
} 

// Compute Y' 
double SodiumCurrent::ComputeMPrime(double voltage, double m, double h, double j)
{    
    UpdateGatingVariables(m, h, j);
    UpdateAlphaAndBeta(voltage);
    return  (mAlphaM - (mAlphaM + mBetaM)*mM); 
}

double SodiumCurrent::ComputeHPrime(double voltage, double m, double h, double j)
{    
    UpdateGatingVariables(m, h, j);
    UpdateAlphaAndBeta(voltage);
    return  (mAlphaH - (mAlphaH + mBetaH)*mH);
}


double SodiumCurrent::ComputeJPrime(double voltage, double m, double h, double j)
{       
    UpdateGatingVariables(m, h, j);
    UpdateAlphaAndBeta(voltage);
    return  (mAlphaJ - (mAlphaJ + mBetaJ)*mJ);
}


