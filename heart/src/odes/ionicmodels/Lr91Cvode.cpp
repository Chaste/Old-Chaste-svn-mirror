...
//
const double Lr91Cvode::membrane_C = 1.0;
const double Lr91Cvode::membrane_F = 96484.6;
const double Lr91Cvode::membrane_R = 8314;
...
const double Lr91Cvode::ionic_concentrations_Nao = 140.0;
const double Lr91Cvode::plateau_potassium_current_g_Kp = 0.0183;
const double Lr91Cvode::time_dependent_potassium_current_PR_NaK = 0.01833;
 ...
 * Destructor
 */
Lr91Cvode::~Lr91Cvode(void)
{
}

double Lr91Cvode::GetIntracellularCalciumConcentration()
{
    return NV_Ith_S(mStateVariables, 3);
}


void Lr91Cvode::EvaluateRhs(double time, N_Vector y, N_Vector ydot)
{
    double fast_sodium_current_h_gate_h = NV_Ith_S(y,0);
    double fast_sodium_current_j_gate_j = NV_Ith_S(y,1);
    double fast_sodium_current_m_gate_m = NV_Ith_S(y,2);
    double intracellular_calcium_concentration_Cai = NV_Ith_S(y,3);
    double membrane_V = NV_Ith_S(y,4);
    ...
    NV_Ith_S(ydot,0) = fast_sodium_current_h_gate_h_prime;
    NV_Ith_S(ydot,1) = fast_sodium_current_j_gate_j_prime;
    NV_Ith_S(ydot,2) = fast_sodium_current_m_gate_m_prime;
    NV_Ith_S(ydot,3) = intracellular_calcium_concentration_Cai_prime;
    NV_Ith_S(ydot,4) = membrane_V_prime;
    NV_Ith_S(ydot,5) = slow_inward_current_d_gate_d_prime;
    NV_Ith_S(ydot,6) = slow_inward_current_f_gate_f_prime;
    NV_Ith_S(ydot,7) = time_dependent_potassium_current_X_gate_X_prime;
}


double Lr91Cvode::GetIIonic()
{
    double fast_sodium_current_h_gate_h = NV_Ith_S(mStateVariables,0);
    double fast_sodium_current_j_gate_j = NV_Ith_S(mStateVariables,1);
    double fast_sodium_current_m_gate_m = NV_Ith_S(mStateVariables,2);
    double intracellular_calcium_concentration_Cai = NV_Ith_S(mStateVariables,3);
    double membrane_V = NV_Ith_S(mStateVariables,4);
...
void Lr91Cvode::VerifyStateVariables()
{
    const double fast_sodium_current_h_gate_h = NV_Ith_S(mStateVariables,0);            // gating
    const double fast_sodium_current_j_gate_j = NV_Ith_S(mStateVariables,1);            // gating
    const double fast_sodium_current_m_gate_m = NV_Ith_S(mStateVariables,2);            // gating
    const double intracellular_calcium_concentration_Cai = NV_Ith_S(mStateVariables,3); // concentration
    const double slow_inward_current_d_gate_d = NV_Ith_S(mStateVariables,5);            // gating
    const double slow_inward_current_f_gate_f = NV_Ith_S(mStateVariables,6);            // gating
    const double time_dependent_potassium_current_X_gate_X = NV_Ith_S(mStateVariables,7); // gating

    #define COVERAGE_IGNORE
    if (!(-mAbsTol<=fast_sodium_current_h_gate_h && fast_sodium_current_h_gate_h<=1.0+mAbsTol))
    {
    ...
    if (!(-mAbsTol<=fast_sodium_current_j_gate_j && fast_sodium_current_j_gate_j<=1.0+mAbsTol))
    {
    ...
    if (!(-mAbsTol<=fast_sodium_current_m_gate_m && fast_sodium_current_m_gate_m<=1.0+mAbsTol))
    {
    ...
    if (!(-mAbsTol<intracellular_calcium_concentration_Cai))
    {
    ...
    if (!(-mAbsTol<=slow_inward_current_d_gate_d && slow_inward_current_d_gate_d<=1.0+mAbsTol))
    {
    ...
    if (!(-mAbsTol<=slow_inward_current_f_gate_f && slow_inward_current_f_gate_f<=1.0+mAbsTol))
    {
    ...
    if (!(-mAbsTol<=time_dependent_potassium_current_X_gate_X && time_dependent_potassium_current_X_gate_X<=1.0+mAbsTol))
    {
...
template<>
void OdeSystemInformation<Lr91Cvode>::Initialise(void)
{
    // State variables
