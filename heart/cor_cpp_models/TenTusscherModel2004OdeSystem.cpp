...
AbstractOdeSystem(17)
{
   ...
   L_type_calcium_current_f_Ca_gate_tau_f_Ca = 2.0;
   L_type_calcium_current_g_CaL = 0.000175;
   membrane_Cm = 2.0;
   model_parameters_Ca_o = 2.0;
   model_parameters_F = 96.4867;
...
std::vector<double> TenTusscherModel2004OdeSystem::EvaluateYDerivatives (double time, const std::vector<double> &rY)
{
   ...
   if (membrane_V < -40.0)
   {
      fast_sodium_current_h_gate_beta_h = 2.7*exp(0.079*membrane_V)+310000.0*exp(0.3485*membrane_V);
   }
   else
   {
   ...
   if (membrane_V < -40.0)
   {
      fast_sodium_current_j_gate_alpha_j = (-25428.0*exp(0.2444*membrane_V)-6.948E-6*exp(-0.04391*membrane_V))*(membrane_V+37.78)/(1.0+exp(0.311*(membrane_V+79.23)));
   }
   else
   {
   ...
   double reversal_potentials_E_K = model_parameters_R*model_parameters_T/model_parameters_F*log10(model_parameters_K_o/potassium_dynamics_K_i);
   ...
   double L_type_calcium_current_d_gate_alpha_d = 1.4/(1.0+exp((-35.0-membrane_V)/7.5))+0.25;
   double L_type_calcium_current_d_gate_beta_d = 1.4/(1.0+exp((5.0+membrane_V)/5.0));
   double L_type_calcium_current_d_gate_gamma_d = 1.4/(1.0+exp((50.0-membrane_V)/20.0));
   ...
   double L_type_calcium_current_f_Ca_gate_alpha_f_Ca = 1.0/(1.0+exp(pow(calcium_dynamics_Ca_i/0.000325, 8.0)));
   ...
   double membrane_V_prime = -(fast_sodium_current_i_Na+inward_rectifier_potassium_current_i_K1+transient_outward_current_i_to+rapid_delayed_rectifier_current_i_Kr+slow_delayed_rectifier_current_i_Ks+L_type_calcium_current_i_CaL+Na_Ca_exchanger_current_i_NaCa+sodium_potassium_pump_current_i_NaK+sodium_background_current_i_bNa+calcium_background_current_i_bCa+calcium_pump_current_i_pCa+potassium_pump_current_i_pK+membrane_i_stim)/membrane_Cm;
   double potassium_dynamics_K_i_prime = -(inward_rectifier_potassium_current_i_K1+transient_outward_current_i_to+rapid_delayed_rectifier_current_i_Kr+slow_delayed_rectifier_current_i_Ks+potassium_pump_current_i_pK+membrane_i_stim-2.0*sodium_potassium_pump_current_i_NaK)/(model_parameters_Vc*model_parameters_F)*membrane_Cm;
