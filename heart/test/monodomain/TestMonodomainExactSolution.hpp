...
public :
    ...
    double GetIIonic()
    {
        ...
        {
            return 0*cos(M_PI*mX[0])*cos(M_PI*mX[1])*cos(M_PI*mX[2]);
        }
    }
...
public: 
    ...
    double GetStimulus(double time)
    {
        ...
        {
            return -3*M_PI*M_PI*cos(M_PI*mX[0])*cos(M_PI*mX[1])*cos(M_PI*mX[2]);
        }
    }
};
...
public:
    void TestMonodomainExactSolution3d() throw(Exception)
    {
        ...
        HeartConfig::Instance()->SetSimulationDuration(0.05); //ms
...
HeartConfig::Instance()->SetIntracellularConductivities(Create_c_vector(1, 1, 1));
        ...
        for(unsigned timestep=0; timestep<num_timesteps; timestep++)
        {
            ...
            for(unsigned i=0; i<mesh.GetNumNodes(); i++)
            {
                ...
                double exact_solution = cos(M_PI*x)*cos(M_PI*y)*cos(M_PI*z);
                // 5% error tolerance
                TS_ASSERT_DELTA(V, exact_solution, 0.05);
            }
        }
