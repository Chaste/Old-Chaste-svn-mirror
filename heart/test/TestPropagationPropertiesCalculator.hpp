...
public:
    ...
    {
        Hdf5DataReader simulation_data("heart/test/data/Monodomain1d",
                                       "repeated_stimuli", false);
        
        // These data were generated by a monodomain simulation of a 1D cable 
        // of 100 nodes with a space step of 0.1mm.
        // 422 ms were simulated. Two stimuli were applied at node 0, one at 2 ms 
        // and the other at 402.
        // The file contains expeorted data at 4 nodes (5, 20, 40 and 95).
         PropagationPropertiesCalculator ppc(&simulation_data);
         double velocity = 0;
        
        // First we check that no exception is thrown
        ...
        //i.e. from 5 to 95 is equals to the opposite of 95 to 5
        TS_ASSERT_EQUALS(ppc.CalculateConductionVelocity(5,95,0.9),-ppc.CalculateConductionVelocity(95,5,0.9));
        TS_ASSERT_EQUALS(ppc.CalculateAllConductionVelocities(5,95,0.9)[0],-ppc.CalculateAllConductionVelocities(95,5,0.9)[0]);
        
       // Then we check some values (should be uniform over the cable)
        TS_ASSERT_DELTA(ppc.CalculateConductionVelocity(5,20,0.15), 0.05, 0.003);
        TS_ASSERT_DELTA(ppc.CalculateConductionVelocity(20,40,0.2), 0.05, 0.003);
        //faking half euclidian distance, check that it gets twice slower 
        TS_ASSERT_DELTA(ppc.CalculateConductionVelocity(20,40,0.1), 0.025, 0.003);
        TS_ASSERT_DELTA(ppc.CalculateConductionVelocity(5,40,0.35), 0.05, 0.003);
        
        //Finally make sure that the method that gets all the conduction velocities returns the same answers       
        TS_ASSERT_EQUALS(ppc.CalculateAllConductionVelocities(5,40,0.35)[0],ppc.CalculateConductionVelocity(5,40,0.35));
        TS_ASSERT_EQUALS(ppc.CalculateAllConductionVelocities(5,20,0.15)[0],ppc.CalculateConductionVelocity(5,20,0.15));
        TS_ASSERT_EQUALS(ppc.CalculateAllConductionVelocities(40,95,0.45)[0],ppc.CalculateConductionVelocity(40,95,0.45));
