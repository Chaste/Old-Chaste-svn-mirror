...
public:
    BenchmarkCellFactory()
        : AbstractCardiacCellFactory<3>(),
          mpStimulus(new SimpleStimulus(-70000.0, 2))
    {
    }
...
// Update: this test now passes when we run with an ODE timestep of 0.005 instead of 0.01.
...
//        mesh.ConstructRectangularMesh(num_elem_x, num_elem_y);
        ...
        std::cout << extremes(0) << " "
                  ...
                  << extremes(5) << "\n";
          */        
        HeartConfig::Instance()->SetOutputDirectory("TestLuoRudy91FailingSlab");
