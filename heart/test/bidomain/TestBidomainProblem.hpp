...
public:
    ...
    void TestBidomainFallsOverProducesOutput() throw(Exception)
    {
        ...
        // Test no longer throws as the autogenerated model completes a simulation.
        bidomain_problem.Solve();
        ...
        TS_ASSERT_EQUALS( times.size(),  31U);//For over stimulation
        TS_ASSERT_DELTA( times[1], 0.01,  1e-12);
        //TS_ASSERT_DELTA( times.back(), 0.3,  1e-12);//For normal stimulation
        TS_ASSERT_DELTA( times.back(), 0.30,  1e-12);//For over stimulation
