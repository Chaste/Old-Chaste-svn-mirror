...
//
	...
	/*
	* This test creates two vectors and write values into them in a time loop.
	* The aim of this test is to check that it doesn't deadlock in parallel (see #1573)
	*/
	void TestWriterDeadlock() throw (Exception)
	{
		unsigned num_time_steps = 2744;//this should be >= 2744 for this test to be of any use, see #1573

		unsigned num_nodes = 4;
		DistributedVectorFactory* p_factory = new DistributedVectorFactory(num_nodes);
		Hdf5DataWriter writer(*p_factory,
							  "hdf_writer_deadlock_check",
							  "hdf_4_nodes",
							  true,
							  false);

		std::vector<int> variablesIDs;
		int column_ID1 = writer.DefineVariable("ID1","mV");
		int column_ID2 = writer.DefineVariable("ID2","mV");
		int column_ID3 = writer.DefineVariable("ID3","mV");
		variablesIDs.push_back(column_ID1);
		variablesIDs.push_back(column_ID2);
		variablesIDs.push_back(column_ID3);
		writer.DefineFixedDimension( num_nodes );
		writer.DefineUnlimitedDimension("Time","msecs");
		writer.EndDefineMode();

		Vec solution = p_factory->CreateVec(3);
		DistributedVector dist_solution = p_factory->CreateDistributedVector(solution);
		std::vector<DistributedVector::Stripe> stripe;
		stripe.reserve(3);
		for (unsigned i=0; i<3; i++)
		{
		   stripe.push_back(DistributedVector::Stripe(dist_solution, i));
		}
		...
		for (DistributedVector::Iterator index = dist_solution.Begin();
			...
			++index)
		{
		   stripe[0][index] = 56.0;//some numbers...
		   stripe[1][index] = 57.0;
		   stripe[2][index] = 58.0;
		}
		dist_solution.Restore();

		for (unsigned time_step = 0; time_step < num_time_steps; time_step++)
		{
			writer.PutUnlimitedVariable((double) time_step);

			Vec vector =  p_factory->CreateVec(3);
			DistributedVector dist_vector = p_factory->CreateDistributedVector(vector);

		   DistributedVector::Stripe dist_vector_first_stripe(dist_vector,0);
		   DistributedVector::Stripe dist_vector_second_stripe(dist_vector,1);
		   DistributedVector::Stripe dist_vector_third_stripe(dist_vector,2);

		   DistributedVector::Stripe solution_first_stripe(dist_solution,0);
		   DistributedVector::Stripe solution_second_stripe(dist_solution,1);
		   DistributedVector::Stripe solution_third_stripe(dist_solution,2);

		   for (DistributedVector::Iterator index = dist_vector.Begin();
				index != dist_vector.End();
				++index)
		   {
				dist_vector_first_stripe[index] = 1.0 + solution_first_stripe[index];//some functions of the striped solution
				dist_vector_second_stripe[index] = 2.0 + solution_second_stripe[index];
				dist_vector_third_stripe[index] = 3.0 + solution_third_stripe[index];
		   }

			dist_vector.Restore();
			writer.PutStripedVector(variablesIDs, vector);
