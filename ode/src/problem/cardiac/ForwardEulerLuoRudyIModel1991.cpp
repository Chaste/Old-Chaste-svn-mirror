 ...
 * Constructor
 */
ForwardEulerLuoRudyIModel1991::ForwardEulerLuoRudyIModel1991(double dt,
 ...
 * Destructor
 */
ForwardEulerLuoRudyIModel1991::~ForwardEulerLuoRudyIModel1991(void)
{
    // Do nothing
}

void ForwardEulerLuoRudyIModel1991::Init()
{
...
OdeSolution ForwardEulerLuoRudyIModel1991::Compute(double tStart, double tEnd)
{
    ...
    for (int i=0; i<n_steps; i++)
    {
        ...
        double i_stim = GetStimulus(curr_time);
...
OdeSolution ForwardEulerLuoRudyIModel1991::ComputeExceptVoltage(double tStart, double tEnd)
{
    // This method iterates over timesteps, calling ComputeExceptVoltage(t) at
    // each one, to update all state variables except for V, using forward Euler.
    double _n_steps = (tEnd - tStart) / mDt;
    int n_steps = (int) round(_n_steps);
    assert(fabs(n_steps - _n_steps) < 1e-12);
...
void ForwardEulerLuoRudyIModel1991::ComputeExceptVoltage(double tStart)
{
    ...
    // update h
    rY[0] += fast_sodium_current_h_gate_h_prime*mDt;
                                   
    // update j
    rY[1] += fast_sodium_current_j_gate_j_prime*mDt;

    // update m
    rY[2] += fast_sodium_current_m_gate_m_prime*mDt;

    // update d
    rY[5] += slow_inward_current_d_gate_d_prime*mDt;

    // update f
    rY[6] += slow_inward_current_f_gate_f_prime*mDt;

    // update X
    rY[7] += time_dependent_potassium_current_X_gate_X_prime*mDt;


    // finally, calculate the Calcium concentration at the next time
    double intracellular_calcium_concentration_Cai = rY[3];
    ...
    rY[3] += intracellular_calcium_concentration_Cai_prime*mDt;
}
 ...
 */
std::vector<double> ForwardEulerLuoRudyIModel1991::EvaluateYDerivatives (double time, const std::vector<double> &rY)
{
    double fast_sodium_current_h_gate_h = rY[0];
    double fast_sodium_current_j_gate_j = rY[1];
    double fast_sodium_current_m_gate_m = rY[2];
...
double ForwardEulerLuoRudyIModel1991::GetIIonic()
{
...
void ForwardEulerLuoRudyIModel1991::VerifyGatingVariables()
{
#ifndef NDEBUG
