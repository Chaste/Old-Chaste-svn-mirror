...
##                print key, repr(root_comp), repr(cur_comp), repr(cur_comp.parent(reln_key=key))
    ...
    def _topological_sort(self, node):
        ...
        We want to also keep track of the order in which components
        are visited, so that the sort order may be used when
        generating program code in a modular style, rather than just a
        flattened version.  This isn't done yet however.
        """
    ...
    def _add_solver_info(self, force=False):
        ...
        """
        if force and hasattr(self, u'solver_info'):
            self.xml_remove_child(self.solver_info)
        if hasattr(self, u'solver_info'):
            solver_info = self.solver_info
        else:
            solver_info = self.xml_create_element(u'solver_info',
                                                  NSS[u'solver'])
            self.xml_append(solver_info)
        # The name of the transmembrane potential
        if not hasattr(solver_info, u'transmembrane_potential'):
            v_elt = self.xml_create_element(
                u'transmembrane_potential', NSS[u'solver'],
                content=self._cml_transmembrane_potential.fullname())
            solver_info.xml_append(v_elt)
        self._add_solver_info_ionic_current()
        # Linearised ODEs - where du/dt = g + hu
        # (and g, h are not functions of u)
        if not hasattr(solver_info, u'linear_odes'):
            odes_elt = self.xml_create_element(u'linear_odes', NSS[u'solver'])
            solver_info.xml_append(odes_elt)
            odes_math = self.xml_create_element(u'math', NSS[u'm'])
            odes_elt.xml_append(odes_math)
            linear_vars = self._cml_linear_update_exprs.keys()
            linear_vars.sort(key=lambda v: v.fullname())
            free_var = self._cml_free_var
            for var in linear_vars:
                g, h = self._cml_linear_update_exprs[var]
                hu = mathml_apply.create_new(self, u'times',
                                             [h, var.fullname()])
                rhs = mathml_apply.create_new(self, u'plus', [g, hu])
                odes_math.xml_append(mathml_diff.create_new(
                    self, free_var.fullname(), var.fullname(), rhs))
        # Jacobian matrix elements
        if not hasattr(solver_info, u'jacobian'):
            jac_elt = self.xml_create_element(u'jacobian', NSS[u'solver'])
            solver_info.xml_append(jac_elt)
            jac_vars = self._cml_jacobian.keys()
            ...
            for v_i, v_j in jac_vars:
                ...
                attrs = {u'var_i': unicode(v_i),
                         u'var_j': unicode(v_j)}
                entry = self.xml_create_element(u'entry', NSS[u'solver'],
                                                attributes=attrs)
                jac_elt.xml_append(entry)
                entry_doc = amara_parse(self._cml_jacobian[(v_i, v_j)].xml(),
                                        rules=rules, binderobj=binder)
                entry.xml_append(entry_doc.math)
        return

    def _add_solver_info_ionic_current(self):
        ...
        # configuration store.
        if hasattr(self.xml_parent, '_cml_config') and \
            ...
            for var in conf.i_ionic_vars:
                DEBUG("translate", var.name, var.xml_parent.name,
                      var.fullname())
                varelt = self.xml_create_element(u'var', NSS[u'solver'],
