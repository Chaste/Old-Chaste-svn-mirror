...
private:
    ...
    void TestSimpleNodeBasedTissue(std::string meshFilename)
    {
        ...
        unsigned counter = 0;
        for (typename NodeBasedTissue<DIM>::Iterator cell_iter = node_based_tissue.Begin();
...
public:
    ...
    void TestRemoveDeadCellsAndUpdate()
    {
        ...
        unsigned index = 0;
        for (NodeBasedTissue<2>::Iterator cell_iter = node_based_tissue.Begin();
             cell_iter != node_based_tissue.End();
             ++cell_iter)
        {
    ...
    void TestGetLocationOfCell() throw (Exception)
    {
             ...
             ++cell_iter)
        {
            ...
            TissueCell& r_cell = node_based_tissue.rGetCellUsingLocationIndex(cell_iter.GetNode()->GetIndex());
    ...
    void TestArchivingTissue() throw (Exception)
    {
        ...
        // Archive a simple tissue
        {
            ...
            // Cells have been given birth times of 0, -1, -2, -3, -4.
            // loop over them to run to time 0.0;
            for (NodeBasedTissue<2>::Iterator cell_iter = p_tissue->Begin();
