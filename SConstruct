...
###############################################################################
# Start of #244 hackery - finer grained dependency analysis
###############################################################################
env['ORIG_ENV'] = env

def is_template_cpp(filepath):
...
# Builder which works out dependencies of a .cpp or .hpp file
def my_list_filesbuild_function(target, source, env):
    """Find out which object files we need to link with the given test runner .o.

    We look at the list of .hpp files included by the test, using the
    implicit dependencies of the test runner .o file given in target.
    For each one, if it is a Chaste header and if it has a corresponding
    <file>.cpp (and it isn't a template class), we assume we need to link
    against <file>.o.
    """
    global RUNNER_DEP_OBJS
    source_files = []
    header_files = []
    def process(o):
        o.scan()
        #print o.sources, o.implicit, o.has_builder(), o.children(scan=0), o.includes
        #print o.render_include_tree()
        #print o.depends
        for s in o.sources:
            ss = str(s)
            if ss not in source_files:
                source_files.append(ss)
                process(s)
        for d in o.implicit:
            sd = str(d)
            if sd not in header_files:
                header_files.append(sd)
                process(d)

    for o in source:
        process(o)
    our_headers, our_objs, objs = [], [], []
    #for s in source_files:
    #    print 'S', s
    for s in header_files:
        #print 'H', s
        component = s.split(os.path.sep)[0]
        if component in components:
            our_headers.append(s)
            base, ext = os.path.splitext(s)
            cpp = base + '.cpp'
            if os.path.exists(cpp) and not is_template_cpp(cpp):
                rest = base[len(component):]
                objpath = os.path.join(component, 'build', build.build_dir + rest)
                obj = env['ORIG_ENV'].StaticObject(source=cpp, target=objpath)
                process(obj[0])
                our_objs.append(base)
                objs.append(obj)
                #rest = base[len(component):]
                #obj = File(component + '/build/' + build.build_dir + rest + '.o')
                env.Depends(env['RUNNER_EXE'], obj)
                #env['RUNNER_EXE'].add_source(obj)
    RUNNER_DEP_OBJS = Flatten(objs)
    # Write findings to target
    ...
    return None
env['BUILDERS']['ListFiles'] = Builder(action = my_list_filesbuild_function)

import SCons.Defaults
def our_program_generator(source, target, env, for_signature):
    #print "TestProgram generator."
    #print '\tS:', map(str, source), \
    #    '\n\tD:', map(str, RUNNER_DEP_OBJS), \
    #    '\n\tT:', map(str, target)
    s = set(source)
    for dep in RUNNER_DEP_OBJS:
        if dep not in s:
            source.append(dep)
            s.add(dep)
    #source.extend(RUNNER_DEP_OBJS)
    #print ' New S:', map(str, source)
    #env['SOURCE'] = source
    return SCons.Defaults.LinkAction
our_program = Builder(generator=our_program_generator,
                      prefix = '$PROGPREFIX',
                      suffix = '$PROGSUFFIX',
                      src_suffix = '$OBJSUFFIX',
                      src_builder = 'Object',
                      target_scanner = SCons.Defaults.ProgScan)
env['BUILDERS']['TestProgram'] = our_program
