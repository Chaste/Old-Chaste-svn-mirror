...
*/
#ifndef VERTEXBASEDTISSUESIMULATIONARCHIVER_HPP_
#define VERTEXBASEDTISSUESIMULATIONARCHIVER_HPP_
...
class VertexBasedTissueSimulationArchiver
{
public:
...
template<unsigned DIM, class SIM>
std::string VertexBasedTissueSimulationArchiver<DIM, SIM>::GetArchivePathname(const std::string& rArchiveDirectory, const double& rTimeStamp)
{
...
template<unsigned DIM, class SIM>
SIM* VertexBasedTissueSimulationArchiver<DIM, SIM>::Load(const std::string& rArchiveDirectory, const double& rTimeStamp)
{
    std::string archive_filename = VertexBasedTissueSimulationArchiver<DIM, SIM>::GetArchivePathname(rArchiveDirectory, rTimeStamp);
...
template<unsigned DIM, class SIM>
void VertexBasedTissueSimulationArchiver<DIM, SIM>::Save(SIM* pSim)
{
    ...
    if (dynamic_cast<VertexBasedTissue<DIM>*>(&(pSim->rGetTissue())))
    {
        // The false is so the directory isn't cleaned
        VertexMeshWriter<DIM,DIM> mesh_writer(archive_directory, mesh_filename, false);
        mesh_writer.WriteFilesUsingMesh((static_cast<VertexBasedTissue<DIM>*>(&(pSim->rGetTissue())))->rGetMesh());
    }
...
#endif /*VERTEXBASEDTISSUESIMULATIONARCHIVER_HPP_*/
